describe DifferenceProcessor do
  ...
  describe 'perfom' do
    ...
    it 'should only store the value is different than last one' do
      ...
      DataStore.should_receive(:create).with(:value => 0.21, :identified_by => @feed.id)
      Feed.should_receive(:update).with(@feed.id, :last_value => 252.76)
    ...
    it 'should store the value for the very first time' do
      ...
      DataStore.should_receive(:create).with(:value => 252.76, :identified_by => @feed.id)
      Feed.should_receive(:update).with(@feed.id, :last_value => 252.76)
      processor = DifferenceProcessor.new(252.76, @feed.id)
